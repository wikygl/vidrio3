...
class DisenoNovaPantallaTest {
    ...
    @Test
    fun agregar_un_fijo_agrega_un_modulo() {
        ActivityScenario.launch<DisenoNovaActivity>(intentCon(null)).use { esc ->
            esperar()
            val antes = enPantalla(esc) { DisenoNova.desdePaquete(it.paqueteParaPruebas())!!.nModulos }
            enPantalla(esc) { it.seleccionarParaPruebas(franja = 0, tramo = 0, modulo = 0) }
            esperar(300)
            enPantalla(esc) { it.pulsarFijoParaPruebas() }
            esperar()
            val despues = enPantalla(esc) { DisenoNova.desdePaquete(it.paqueteParaPruebas())!!.nModulos }
            assertEquals("agregar un fijo no agregó nada", antes + 1, despues)
        }
    }

    @Test
    fun agregar_una_corrediza_agrega_un_modulo() {
        ActivityScenario.launch<DisenoNovaActivity>(intentCon(null)).use { esc ->
            esperar()
            val antes = enPantalla(esc) { DisenoNova.desdePaquete(it.paqueteParaPruebas())!!.nModulos }
            enPantalla(esc) { it.seleccionarParaPruebas(franja = 0, tramo = 0, modulo = 0) }
            esperar(300)
            enPantalla(esc) { it.pulsarCorredizaParaPruebas() }
            esperar()
            val d = enPantalla(esc) { DisenoNova.desdePaquete(it.paqueteParaPruebas())!! }
            assertEquals("agregar una corrediza no agregó nada", antes + 1, d.nModulos)
            assertTrue("no aparece la corrediza", d.nCorredizas >= 1)
        }
    }

    @Test
    fun partir_el_tramo_crea_dos_tramos() {
        val paquete = "{nova,apa,[240,200:Tl<240>(s<150>(f<60>c<60>c<60>f<60>);m<50>(f<120>f<120>))]}"
        ActivityScenario.launch<DisenoNovaActivity>(intentCon(null)).use { esc ->
            esperar()
            enPantalla(esc) { it.cargarParaPruebas(paquete) }
            esperar()
            enPantalla(esc) { it.seleccionarParaPruebas(franja = 0, tramo = 0, modulo = 1) }
            esperar(300)
            enPantalla(esc) { it.pulsarParanteParaPruebas() }
            esperar()
            val d = enPantalla(esc) { DisenoNova.desdePaquete(it.paqueteParaPruebas())!! }
            assertEquals("el parante no partió el tramo", 2, d.nTramos)
            assertEquals("se perdieron módulos", 4, d.nModulos)
